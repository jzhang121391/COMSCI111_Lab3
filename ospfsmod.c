...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		...
		//if we are checking at a position greater than the directory size, break
		if (f_pos > OSPFS_DIRENTRY_SIZE + 2)
		{
			break;
		}
		...
		ok_so_far = filldir(dirent, od->od_name, sizeof(od->od_name),
		 f_pos, dir_inode->i_ino, file_type);
...
allocate_block(void)
{
	...
	while(block_idx < ospfs_super->os_nblocks) {
		 if(bitvector_test(ospfs_block(2), block_idx) == 1)
		{
			bitvector_clear(ospfs_block(2), block_idx);
			return block_idx;
		}
...
{
	/* EXERCISE: Your code here */
	if((blockno < ospfs_super->os_nblocks) && (blockno > 4))
...
{
	//if the block is one of the first 10 blocks, return b
	if (indir_index(b) == -1)
	{
		return b;
	}
...
add_block(ospfs_inode_t *oi)
{
	...
	uint32_t * indirect_temp;
	
	/* EXERCISE: Your code here */
	
	//if we are trying to add a direct block
	if (n < OSPFS_NDIRECT)
	{
		//try to allocate a block
		temp_block = allocate_block();
		
		//if disk is full return -ENOSPC
		if (temp_block == 0)
		{
			return -ENOSPC;
		}
		...
		{
			//set initial value for the data block to zero
			memset(ospfs_block(temp_block), 0, 1024);
			
			//set the direct block to the allocated block
			oi->oi_direct[n] = temp_block;
			
			return 0;
		}
	}
	//else we check if n is part of the first indirect block
	else if (n >= OSPFS_NDIRECT && n < OSPFS_NINDIRECT + OSPFS_NDIRECT)
	{
		//a new indirect block has not been allocated yet
		if (n == OSPFS_NDIRECT)
		{
			temp_block = allocate_block();
			...
			else
			{
				...
				//set the indirect block to the allocated block
				
				allocated[0] = temp_block;
				
				//we need to allocate again for the direct block
				temp_block = allocate_block()
				...
				{
				oi->oi_indirect= temp_block;
				//set initial value for the data block to zero
				memset(ospfs_block(temp_block), 0, 1024);
				indirect_temp = ospfs_block(oi->oi_indirect);
				indirect_temp[0] = temp_block;
				}
				
			}	
		}
		//we are checking within the first indirect block that has already been allocated
		else
		{
			//get the block offset in the indirect block
			block_offset = direct_index(n);
			...
			{
				memset(ospfs_block(temp_block), 0, OSPFS_BLKSIZE);
				indirect_temp = ospfs_block(oi->oi_indirect);
				indirect_temp[block_offset] = temp_block;	
			}
	...
	{
		//we check if the first indirect block is just full,
		//and we need to use the doubly indirect block
		if (n == OSPFS_DIRECT + OSPFS_NINDIRECT)
		{
			...
			//store it in the first allocated array space
			allocated[0] = temp_block;
			
			//allocate the indirect block within it
			temp_block = allocate_block();
			...
			allocated[1] = temp_block;
			
			//allocate the direct block within it
			temp_block = allocate_block();
			...
			//we need to zero out the blocks we created
			memset(ospfs_block(allocated[0]), 0, OSPFS_BLKSIZE);
			memset(ospfs_block(allocated[1]), 0, OSPFS_BLKSIZE);
			memset(ospfs_block(temp_block), 0, OSPFS_BLKSIZE);
			
			//set the doubly indirect block
			oi->oi_indirect2 = allocated[0];
			
			//set the indirect block within it
			indirect_temp = ospfs_block(oi->oi_indirect2);
			indirect_temp[0] = allocated[1];
			
			//set the direct data block within th indirect
			indirect_temp = ospfs_block(allocated[1]);
			indirect_temp[0] = temp_block;
		}
		//else we set the doubly indirect block that has already been made
		{
			//one of the indirect blocks within the doubly indirect block has just been used
			if (direct_index(n) == 0)
			{
				...
				memset(ospfs_block(allocated[0]), 0, OSPFS_BLKSIZE);
				memset(temp_block, 0, OSPFS_BLKSIZE);
				
				indirect_temp = ospfs_block(oi->oi_indirect2);
				indirect_temp[indir_index(n)] = allocated[0];
				
				indirect_temp = ospfs_block(allocated[0]);
				indirect_temp[0] = temp_block;
			}
			//doubly indirect and indirect blocks are set
			else
			{
				temp_block = allocate_block();
				...
				uint32_t* indirect_temp2 = indirect_temp[indir_index(n)];
				indirect_temp2[direct_index(n)] = temp_block;
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
	...
	ospfs_direntry_t *blank_dir = create_blank_direntry(ospfs_inode(dir->i_ino));
	
	if(IS_ERR(blank_dir))
		return PTR_ERR(blank_dir);

	blank_dir->od_ino = src_dentry->d_inode->i_ino;
	memcpy(blank_dir->od_name, dst_dentry->d_name.name, dst_dentry->d_name.len);
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
	...
	ospfs_inode_t *blank_inode = NULL;
	while(entry_ino < ospfs_super->os_ninodes) {
		if(ospfs_inode(entry_ino)->oi_nlink == 0)
		{
			blank_inode = ospfs_inode(entry_ino);
			
			blank_inode->oi_size = 0;
			blank_inode->oi_ftype = OSPFS_FTYPE_REG;
			blank_inode->oi_nlink++;
			blank_inode->oi_mode = mode;
			memset(blank_inode->oi_direct, 0, sizeof(blank_inode->oi_direct[0]) * OSPFS_NDIRECT);
			blank_inode->oi_indirect = 0;
			blank_inode->oi_indirect2 = 0;
	...
	if(blank_inode == NULL)
		...
		blank_dir->od_ino = entry_ino;
		memcpy(blank_dir->od_name, dentry->d_name.name, dentry->d_name.len);
		blank_dir->od_name[dentry->d_name.len] = 0;
	}
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	ospfs_symlink_inode_t *blank_inode;
	
	while(entry_ino < ospfs_super->os_ninodes) {
		if((ospfs_symlink_inode_t*)ospfs_inode(entry_ino)->oi_nlink == 0) {
			blank_inode = (ospfs_symlink_inode_t*)ospfs_inode(entry_ino);
		}
	...
	blank_inode->oi_size = strlen(symname);
	blank_inode->oi_ftype = OSPFS_FTYPE_SYMLINK;    
	blank_inode->oi_nlink++;
	strcpy(blank_inode->oi_symlink, symname);
        
	strncpy(blank_dir->od_name, dentry->d_name.name, dentry->d_name.len);
	blank_dir->od_name[dentry->d_name.len] = 0;
	blank_dir->od_ino = entry_ino;
